...
Modern language models[^transformer] fall into two families, which differ in what they produce:

- **Decoder models** — GPT, Llama, Claude, the models behind ChatGPT and similar tools — produce text, and are the right tool for open-ended tasks: conversation, drafting, summarising, translating. They're very large, rarely runnable on a personal computer, and typically used through an API, as in the [previous tutorial](https://www.css.cnrs.fr/classification-with-generative-llms-and-api-calls/).
- **Encoder models** — BERT and its relatives — produce no text, but an **embedding**: a numerical representation of a text's meaning. This makes them the right tool for tasks with a closed, well-defined output, such as classification, similarity ranking, or clustering. They're much smaller, and will most likely run on your own machine[^7].
...
Both families can annotate a corpus, but they learn your categories differently:

- A decoder has to be instructed every time, via a prompt — you're relying on it interpreting your instructions correctly.
- An encoder is **fine-tuned** on a few hundred texts you label yourself — it learns from your judgment calls, not a description of them.

That upfront annotation effort — and a resulting model that only does this one task — is the price of an encoder, in exchange for a model you fully control. This has concrete consequences for cost, confidentiality and reproducibility, covered next.

### Pros and cons for social science research{#sec-pros-cons}

- **Coding skills**: fine-tuning requires more code than an API call — a chance to build skills, at the cost of time. [ActiveTigger](css.cnrs.fr/active-tigger) offers a code-free alternative for collaborative annotation and fine-tuning.

<!--TODO add logo-->

- **Hardware and cost**: fine-tuning needs a machine to run on; API calls need a paid key. Neither is equally accessible to everyone.
- **Control and reproducibility**: most providers disclose neither the data their models were trained on nor future changes to them, which limits interpretability. Since you own the encoder and its fine-tuning data, your results stay reproducible and easier to interpret (XXX additional techniques to probe the model???).
- **Open weights**: most generative models are "closed weights" — usable only through an API — while HuggingFace models, encoder or generative, are "open weights" and downloadable, which matters for reproducibility (source with varying performance through after different model updates).
- **Confidentiality**: sensitive corpora cannot go through an external API; you would need to run any model, generative or encoder, on a trusted server. An encoder's lighter hardware needs make that bar easier to clear.
...
One last practical property: the **maximum context window**, the maximum[^max-window-size-actually] number of tokens[^tokens-def] a model can process at once — anything beyond is simply ignored. Older models had short windows (128 to 512 tokens), so long texts had to be split into sentences or paragraphs.
...
## What the hyperparameters are for{#sec-hyperparameters-description}

In @sec-training-parameters we set a number of hyperparameters without saying much about them. Rather than describing each one in isolation, we present them here through the problems they are meant to solve, because this is how you will encounter them in practice: you run a fine-tuning, you observe that something went wrong, and you change the hyperparameter that addresses it.

To follow this section, one mental image is enough. Fine-tuning proceeds by small successive corrections: the model reads a **batch** of annotated texts, compares its predictions with your annotations, and adjusts its parameters slightly in the direction that reduces the gap. It then moves on to the next batch, and repeats until it has gone through the entire training set — that is one **epoch**. The hyperparameters below govern *how many* texts the model reads before each correction, *how large* each correction is, and *how many times* the model goes through your data.

**Problem 1: the model has learned nothing**

At the end of the fine-tuning, the metrics are barely better than chance and the losses (@sec-hyperparameters-tuning) have hardly moved. The corrections applied to the model were either too small to have any effect, or too few.
...
The model performs very well on the texts it was trained on, and noticeably worse on the dev set. It has memorised your training examples instead of learning the distinction you are after. This is called **overfitting**, and it is the most common failure in this kind of project, particularly when annotations are few.
...
Large batches are desirable, but the whole batch must be held in memory at once, and this is usually where a personal computer gives up (see @sec-GPU). The way around it is to accumulate several small batches before applying a correction, which is almost equivalent and requires far less memory:
...
## Read the learning curve{#sec-hyperparameters-tuning}

In @sec-essentials we fine-tuned a BERT model and saved checkpoints containing crucial information; one of them is the lerning curve. The learning curve provides the evolution of the model's performance using the loss function[^lossFunction] evaluated on the train set and the train-eval set. We can retrieve it by using the logs in the `trainer_state.json`; plot it: 
...
- Increase batch size in order to smooth training over more examples per training step.
- Increase weight decay to prevent overfitting.
- Annotate more , you may not have enough examples for the model to learn from.
...
- Increase learning rate for faster weight updates.
- Decrease batch size to increase the number of training steps.
- Decrease weight decay, too much regularization can prevent proper learning.
...
- Increase the number epochs to increase the number of training steps.
- Increase the learning rate for faster weight updates.
- Decrease the batch size to increase the number of training steps.
- Decrease weight decay, too much regularization can prevent proper learning.
...
- Decrease the learning rate for gentlier weight update.
- Increase the batch size in order to smooth training over more examples per training step.
- Increase the weight decay to prevent overfitting.
- Annotate more data, you may not have enough examples for the model to learn from.
