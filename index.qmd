...
That upfront annotation effort, and a model that only ever performs this one task, is the price you pay for an encoder — but in exchange, you keep full control over it. This has concrete consequences for cost, confidentiality, reproducibility and even measurement quality, which we discuss next.
...
- **Measurement quality**: the choice isn't just a matter of convenience — it can bias your conclusions. Ashwin et al. (2025) compared the two approaches on interviews with Rohingya refugees and their Bengali hosts: a generative LLM's coding errors were not random, but systematically correlated with respondents' characteristics, whereas a model fine-tuned on a subset of human-coded transcripts showed less measurement error and bias overall.
...
:::{.callout-note title="How to choose a model" collapse="true"}
...
This section is only useful if you have access to a GPU; if you don't, training on CPU works exactly as shown earlier, just slower, and you can skip ahead to @sec-errors. If you have a NVIDIA GPU[^2] and have set up your cuda[^3] environment, or a Mac with an MX chip, you can accelerate training with cuda or mps respectively.
...
:::{.callout-note title="Dataset formatting" collapse="true"}
...
:::{.callout-note title="Memory overflow" collapse="true"}
...
:::{.callout-note title="Tensor shapes" collapse="true"}
...
:::{.callout-note title="Missing keys when saving/reloading a checkpoint" collapse="true"}
...
Ashwin, J., Chhabra, A., & Rao, V. (2025). Using Large Language Models for Qualitative Analysis can Introduce Serious Bias. *Sociological Methods & Research*, 55(3), 795–839. https://doi.org/10.1177/00491241251338246 
